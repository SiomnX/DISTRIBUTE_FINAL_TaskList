...
export default function UpdateGroupModal({ isOpen, onClose, onCreate }: GroupModalProps) {
    const [groupName, setGroupName] = useState('');
    const [inputGroupId, setInputGroupId] = useState('');

  const handleConfirm = () => {
    if (groupName.trim()) {
      onCreate(Number(inputGroupId), groupName.trim());
      setInputGroupId
      setGroupName('');
      onClose();
    }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg w-96 shadow-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">更改群組名稱</h2>
        ...
        />
        <label className="block text-sm font-semibold text-gray-700 mb-1">新名稱</label>
        <input
          type="text"
          value={groupName}
