...
export default function DeleteGroupModal({ isOpen, onClose, onCreate }: GroupModalProps) {
  const [groupId, setGroupId] = useState('');

  const handleConfirm = () => {
    if (groupId.trim()) {
      onCreate(Number(groupId.trim()));
      setGroupId('');
      onClose();
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg w-96 shadow-md">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">刪除群組</h2>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-700 text-xl">&times;</button>
        </div>

        <label className="block text-sm font-semibold text-gray-700 mb-1">群組ID</label>
        <input
          type="text"
          value={groupId}
